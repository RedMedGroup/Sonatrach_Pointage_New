    ...
    public partial class Frm_FichePost : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void TreeList1_FocusedNodeChanged(object sender, DevExpress.XtraTreeList.FocusedNodeChangedEventArgs e)
        {
            ...
            if (int.TryParse(e.Node.GetValue("ID").ToString(), out id))
            {
                ...
                un = db.Fiche_DePosts.Single(x => x.ID == id);
        ...
        bool IsValidit()
        {
            ...
            var db = new DAL.DataClasses1DataContext();
            var OL = db.Fiche_DePosts.Where(x => x.Name.Trim() == txt_name.Text.Trim() && x.ID != un.ID);
            if (OL.Count() > 0)
            {
        ...
        void RefrechData()
        {
            ...
            treeList1.DataSource = db.Fiche_DePosts;
            treeList1.ExpandAll();
        }
        void New()
        {
            un = new DAL.Fiche_DePost();
        }
        void GetData()
        {
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (un.ID == 0)
            {
                db.Fiche_DePosts.InsertOnSubmit(un);
            }
            else
            {
                db.Fiche_DePosts.Attach(un);
            }
            SetData();
            db.SubmitChanges();
            RefrechData();
